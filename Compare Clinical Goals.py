...
# Get the current patient, case and plan
try:
    patient = get_current('Patient')
    case = get_current('Case')
    plan = get_current('Plan')
except Exception as e:
    print('Patient and plan have to be loaded. Exiting script.')
    print(f'Error: {e}')
    sys.exit()
...
filename = f"S:\\Cancer Services - Radiation Physics\\Auto-planning\\MVision\\Scripts\\clinical_goals_export_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

try:
    with open(filename, "w") as f:
        for i in range(number_of_goals):
            f.write(f"{results[i,0]}, {results[i,1]}, {results[i,2]:.2f}, {results[i,3]:.2f}, {results[i,4]:.2f}, {results[i,5]}\r\n")
